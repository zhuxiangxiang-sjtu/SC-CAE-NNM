...
flow-field reduction, latent-space dynamics prediction, full-field viscous-force
evaluation, and strong fluid-structure coupling iterations.

## Reference

Xiangxiang Zhu and co-authors, "Strong-coupling convolutional autoencoder
nonlinear normal mode framework for fluid-structure interaction prediction,"
under publication.

The final journal citation, DOI, and article link will be added after
publication.
...
## Viscous Force and Structure Update

The structural dynamics update is implemented in `viscous_force.py`.

For each decoded flow state, the module:

1. converts normalized `u`, `v`, and `p` back to physical values;
2. computes full-field velocity gradients on the curvilinear grid;
3. extracts the cylinder-surface values from the first grid column;
4. computes pressure lift and viscous lift;
5. updates structural acceleration, velocity, and displacement.

This replaces the earlier practice of directly loading viscous-force data and
makes the strong-coupling process explicit in the sample code.
...
- The legacy single-file script is retained only as a reference; use the
  modular files for new experiments.

## Requirements File

The repository includes the following `requirements.txt` file:
